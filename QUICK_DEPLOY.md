...
- ❌ Только 1 раз в день (нельзя каждый час!)
...
**Рекомендация для Hobby плана:**
Используйте **GitHub Actions** (уже настроено в `.github/workflows/telegram-cron.yml`):

1. GitHub → Settings → Secrets and variables → Actions
2. Добавьте секреты:
   - `APP_URL`: `https://your-app.vercel.app`
   - `TELEGRAM_CRON_SECRET`: ваш секретный ключ
3. Готово! GitHub Actions будет запускать cron job каждый час бесплатно ✅

**Подробнее:** см. `VERCEL_CRON_LIMITS.md`

---

**Если используете Pro план:**
Vercel автоматически настроит cron job из `vercel.json`:
- Проверьте: Settings → Cron Jobs
- Должен быть виден cron job для `/api/telegram/send-cards`
